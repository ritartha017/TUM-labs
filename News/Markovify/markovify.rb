...
require_relative "dataset_reader"
...
text = Dataset.get_data
@obj = Markovify.new(text: text, order: ARGV[0].to_i)
ngrams_order, ngrams_size = ARGV[0].to_i, ARGV[1].to_i
ngrams_order = 1 if ARGV[0].length > 0
ngrams_size = 30 if ARGV[1].length > 0
puts @obj.pp_markov_it(ngrams_size)